...
import { Medal, Award, Droplets } from 'lucide-react';
...
interface ClientesRankingTotalProps {
  clientesRanking: ClienteConLitros[];
  loading: boolean;
}

const ClientesRankingTotal: React.FC<ClientesRankingTotalProps> = ({ clientesRanking, loading }) => {
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="w-36 text-right">Litros Totales</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {rankingOrdenado.length > 0 ? (
              rankingOrdenado.map((cliente, index) => (
                <TableRow key={cliente.id} className={index < 3 ? "bg-amber-50" : ""}>
                  ...
                  <TableCell className="text-right">
                    <Badge variant="outline" className="bg-blue-50">
                      {cliente.numeroRecogidas}
                    </Badge>
                  </TableCell>
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end">
                      <span className="font-bold text-asram">{cliente.litrosRecogidos}</span>
                      <Droplets className="h-4 w-4 ml-1 text-asram" />
                    </div>
                  </TableCell>
                </TableRow>
              ))
...
export default ClientesRankingTotal;
